...
public class LeftDistanceWidgetConfigureActivity extends Activity implements DialogInterface.OnDismissListener {
    ...
    private static final String PREF_DISTANCE_KEY = "appwidget_distance_";
    ...
    public static void saveDistancePref(Context context, int appWidgetId, int distance) {
        SharedPreferences.Editor prefs = context.getSharedPreferences(PREFS_NAME, 0).edit();
        prefs.putInt(PREF_DISTANCE_KEY + appWidgetId, distance);
        prefs.apply();
    }
    ...
    static int loadIConPref(Context context, int appWidgetId) {
        ...
        if (iconValue != -1) {
            ...
            return new Coloring(context).getMarkerStyle();
        }
    }
