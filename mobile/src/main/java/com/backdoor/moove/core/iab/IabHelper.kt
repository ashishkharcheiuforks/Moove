...
package com.backdoor.moove.core.iab
...
(ctx: Context, base64PublicKey: String) {
    ...
    fun startSetup(listener: OnIabSetupFinishedListener?) {
        ...
        checkNotDisposed()
        if (mSetupDone) throw IllegalStateException("IAB helper is already set up.")
        ...
        if (!mContext!!.packageManager.queryIntentServices(serviceIntent, 0).isEmpty()) {
    ...
    private fun checkNotDisposed() {
        if (mDisposed) throw IllegalStateException("IabHelper was disposed of, so it cannot be used.")
    }
     ...
     * [Activity.onActivityResult] method, at which point you must call
     ...
     * as in [Activity.startActivityForResult]).
     * @param listener The listener to notify when the purchase process finishes
     * @param extraData Extra data (developer payload), which will be returned with the purchase data
     * when the purchase completes. This extra data will be permanently bound to that purchase
     * and will always be returned when the purchase is queried.
     */
    fun launchPurchaseFlow(act: Activity, sku: String, itemType: String, requestCode: Int,
                           listener: OnIabPurchaseFinishedListener?, extraData: String) {
        checkNotDisposed()
     ...
     */
    fun queryInventoryAsync(querySkuDetails: Boolean,
    ...
    companion object {
        ...
        val RESPONSE_GET_SKU_DETAILS_LIST = "DETAILS_LIST"
        val RESPONSE_BUY_INTENT = "BUY_INTENT"
        val RESPONSE_INAPP_PURCHASE_DATA = "INAPP_PURCHASE_DATA"
        val RESPONSE_INAPP_SIGNATURE = "INAPP_DATA_SIGNATURE"
        val RESPONSE_INAPP_ITEM_LIST = "INAPP_PURCHASE_ITEM_LIST"
        val RESPONSE_INAPP_PURCHASE_DATA_LIST = "INAPP_PURCHASE_DATA_LIST"
        val RESPONSE_INAPP_SIGNATURE_LIST = "INAPP_DATA_SIGNATURE_LIST"
        val INAPP_CONTINUATION_TOKEN = "INAPP_CONTINUATION_TOKEN"

        // Item types
        val ITEM_TYPE_INAPP = "inapp"
        val ITEM_TYPE_SUBS = "subs"

        // some fields on the getSkuDetails response bundle
        val GET_SKU_DETAILS_ITEM_LIST = "ITEM_ID_LIST"
        val GET_SKU_DETAILS_ITEM_TYPE_LIST = "ITEM_TYPE_LIST"
